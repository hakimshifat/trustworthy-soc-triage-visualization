# GitHub Pages Deployment Checklist

- [x] Add a GitHub Actions workflow that builds and deploys the static application to GitHub Pages.
- [x] Commit and push the deployment workflow to the `main` branch.
- [x] Enable GitHub Pages using the repository’s Actions deployment source.
- [x] Confirm the public Pages URL serves the completed visualization.

## Video-style Animated Demonstration

- [x] Add a guided scene player with play, pause, replay, and scene navigation controls.
- [x] Animate the SOC overload, provenance, calibration, decision-gate, attack, and safety-outcome story beats.
- [x] Preserve research-integrity labels across every animated scene.
- [x] Test the animated player on desktop and mobile, then republish it to GitHub Pages.

## Responsive Animation Repair

- [x] Rebuild the selective-triage scene composition so its gate, routes, and headline never overlap the player controls.
- [x] Verify the corrected scene at the reported compact desktop viewport and republish the live site.

## Animated Data-Masking Explanation

- [x] Add a dedicated animated data-masking section with only invented, safely masked examples.
- [x] Explain the access boundary, sensitive-field detection, stable pseudonymization or approved redaction, semantic-preservation checks, and human review.
- [ ] Label the section clearly as active preparation and “not experiment-ready,” then test and republish it.
